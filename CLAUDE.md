...
- 4단계: 공유 로어북 저장소 (lorebookApi.cjs) + 캐릭터 globalLore 연결 — 현재 상태:
  ...
  - 편집: source_lorebook_id가 있는 항목도 개인 항목과 동일하게 LoreBookData.svelte에서 잠금 없이 자유 편집 — 편집은 항상 캐릭터 자신의 globalLore 복제본(개인 테스트용)에만 반영되고, 공유 원본은 안 건드림. "업로드" 버튼은 연결 안 된 항목이거나(신규 등록) 연결됐지만 로컬 내용이 마지막 업로드/동기화 시점과 달라진("dirty") 항목에만 뜸 — dirty 판정은 loreBook.source_synced_snapshot(contentFingerprint, 내용 필드만 비교, alwaysActive/disabled 제외)과 현재 값 비교. 업로드 시: 잠금 상태를 먼저 read-only로 peek(다른 사용자가 업로드 중이면 확인창 전에 즉시 차단) → "기존 공유 로어북 'OO'을 덮어씁니다" 확인 → uploadEntryToSharedLorebook이 실제 잠금 획득+저장을 원자적으로 수행(peek~confirm 사이 레이스는 SharedLorebookLockedError로 처리). 이게 예전엔 UI에서 도달 불가능했던 "기존 책 덮어쓰기+잠금" 분기였는데 이제 실경로. 서버(lorebookApi.cjs)는 무변경 — lockEntry가 같은 유저 재요청 시 갱신(idempotent)이라 peek 후 즉시 lock+save해도 안전. dirty 상태인 항목이 "받아오기" 대상이 되면: 배경 일괄 반영(applyAllPending)은 조용히 건너뛰고 skippedEntries로 집계, 개별 스파클/버전복원 버튼은 확인창으로 경고 후 진행(force:true). "공유" 뱃지 옆에 dirty면 "미업로드" 뱃지 추가 표시. 버전 기록/복원 버튼은 여기 있음
