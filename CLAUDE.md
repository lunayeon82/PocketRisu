...
- 4단계: 공유 로어북 저장소 (lorebookApi.cjs) + 캐릭터 globalLore 연결 — 현재 상태:
  ...
  - 편집: source_lorebook_id가 있는 항목은 LoreBookData.svelte에서 필드가 읽기 전용이고, "수정" 버튼으로 서버 잠금을 획득해야만 편집 가능(별도 editDraft에서 편집, "저장"=반영+잠금해제, "취소"=로컬 안 건드리고 잠금만 해제) — 예전 SharedLoreBookStore의 잠금-편집 UX를 항목 화면 안으로 옮긴 것. 연결 안 된 개인 항목은 자유 편집 + "업로드" 버튼으로 신규 등록(잠금 불필요, 없던 걸 새로 만드는 거라). "공유" 뱃지와 버전 기록/복원 버튼도 여기 있음
