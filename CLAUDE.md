...
- 4단계: 공유 로어북 저장소 (lorebookApi.cjs) + 캐릭터 globalLore 연결 — 현재 상태:
  ...
  - 연결: loreBook에 source_lorebook_id/source_updated_at/source_synced_snapshot 추가. 로어 항목 1개 = 공유 로어북 1개(entry-level, content:[그 항목]) 대응. sharedLorebookLink.svelte.ts가 업로드(uploadEntryToSharedLorebook, 잠금→저장)/동기화(syncEntriesFromSharedLorebook, 받아오기·업데이트·버전복원이 공용으로 씀)를 담당 — 동기화는 콘텐츠는 서버 값으로 교체하되 alwaysActive/disabled(활성화 방식)는 항상 로컬 값을 보존
  - 백그라운드 동기화: LoreBookSetting.svelte가 Character 탭이 열려있는 동안 15초 폴링(startBackgroundSync/stopBackgroundSync)으로 "아직 안 가져온 새 글로벌 책"/"연결된 항목의 원본이 갱신됨"을 감지해 로어북 화면 상단에 배너("받아오기" 버튼 하나)로 표시, 클릭 시 일괄 반영하고 결과(신규 n건/업데이트 n건, 로컬 미업로드로 건너뛴 n건)를 토스트로 알림
  - 편집: source_lorebook_id가 있는 항목은 LoreBookData.svelte에서 기본 읽기전용 — "편집 시작"(pencil)이 서버 잠금(POST .../lock)을 쥐어야만 필드가 풀림. **잠금은 단일 소유자 배타적**(I/O 독점과 동일 원리)이라 한 명이 쥐고 있는 동안 다른 사람은 편집 시작 자체가 409로 막힘(자기가 이미 쓰던 로컬 콘텐츠는 계속 사용 가능, 새로 편집만 불가). 잠금 해제는 "업로드" 성공 시(서버 saveEntry가 자동 해제) 또는 "편집 취소"(DELETE .../lock, 로컬에 이미 한 수정 내용은 안 지움 — 나중에 다시 업로드 가능) 둘 중 하나뿐 — TTL 없음, 자연 만료 안 됨(놓고 잊으면 진짜로 계속 잠겨있음, 안전장치는 취소 버튼뿐). 잠금 상태에서 실제 편집은 별도 draft가 아니라 캐릭터 자신의 globalLore `value`에 직접 반영(개인 테스트용) — 그대로 dirty/snapshot 추적에 들어감. "업로드" 버튼은 연결 안 된 항목(신규 등록)이거나 연결됐지만 로컬 내용이 마지막 업로드/동기화 시점과 달라진("dirty") 항목에만 뜸 — dirty 판정은 loreBook.source_synced_snapshot(contentFingerprint, 내용 필드만 비교, alwaysActive/disabled 제외)과 현재 값 비교. 업로드는 확인창("기존 공유 로어북 'OO'을 덮어씁니다") 후 uploadEntryToSharedLorebook이 잠금 획득(이미 쥐고 있으면 갱신)+저장을 원자적으로 수행 — 별도 사전 peek 없음(어차피 편집 시작 때 이미 잠갔으므로 자기 자신의 잠금을 되묻는 꼴이라 제거함), 다른 사람이 쥐고 있으면 SharedLorebookLockedError로 처리. 이게 예전엔 UI에서 도달 불가능했던 "기존 책 덮어쓰기+잠금" 분기였는데 이제 실경로. dirty 상태인 항목이 "받아오기" 대상이 되면: 배경 일괄 반영(applyAllPending)은 조용히 건너뛰고 skippedEntries로 집계, 개별 스파클/버전복원 버튼은 확인창으로 경고 후 진행(force:true). "공유" 뱃지 옆에 dirty면 "미업로드" 뱃지 추가 표시. 버전 기록/복원 버튼은 여기 있음
