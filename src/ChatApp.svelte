<script lang="ts">
    import { settingsOpen, openPresetList, openModelPresetList, openModelProfileBrowser, openPersonaList, personaSelectCallback, openHypaV3PresetList, openThemePresetList, MobileGUI, loadedStore, alertStore, LoadingStatusState, bookmarkListOpen, popupStore, popUpEditorStore, botMakerMode, selectedCharID } from './ts/stores.svelte';
    import { DBState } from './ts/stores.svelte';
    import ChatScreen from './lib/ChatScreens/ChatScreen.svelte';
    import ExcelRibbon from './lib/ChatScreens/Excel/ExcelRibbon.svelte';
    import ExcelSheetTabs from './lib/ChatScreens/Excel/ExcelSheetTabs.svelte';
    import CharConfig from './lib/SideBars/CharConfig.svelte';
    import DevTool from './lib/SideBars/DevTool.svelte';
...
<main class="flex bg-bg w-full h-full max-w-100vw text-textcolor" ondragover={(e) => {
    ...
    {:else}
        <div class="flex flex-col w-full h-full">
            <ExcelRibbon
                openGrid={() => {gridOpen = true}}
                devTool={devTool}
                onToggleDevTool={() => { devTool = !devTool; botMakerMode.set(false); }}
            />
            <div class="flex grow min-h-0 overflow-hidden">
                {#if gridOpen}
                    <GridChars endGrid={() => {gridOpen = false}} />
                {:else if $selectedCharID >= 0 && devTool && DBState.db.enableDevTools}
                    <DevTool />
                {:else if $selectedCharID >= 0 && $botMakerMode}
                    <CharConfig />
                {:else}
                    <ChatScreen />
                {/if}
            </div>
            {#if $selectedCharID >= 0 && !gridOpen}
                <ExcelSheetTabs />
            {/if}
        </div>
    {/if}
