<script>
    ...
    import { deleteChatFromServer, saveNewChatToServer } from "src/ts/storage/chatStorage";
    import { v4 } from "uuid";
    ...
    <div class="bg-darkbg p-4 break-any rounded-md flex flex-col max-w-3xl w-72 max-h-full overflow-y-auto">
        ...
        <div class="flex mt-2 items-center">
            <button class="text-textcolor2 hover:text-primary cursor-pointer mr-1" onclick={() => {
                ...
                changeChatTo(0)
                void requestImmediateSave()
                saveNewChatToServer(DBState.db.characters[$selectedCharID].chaId, 0, newChat.id, newChat).catch(() => {})
                close()
            }}>
                <PlusIcon/>
            </button>
