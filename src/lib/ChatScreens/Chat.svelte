<script lang="ts">
    ...
    import { saveNewChatToServer, createChatFolder, updateChatMeta } from "src/ts/storage/chatStorage"
...
{#snippet minorIconButtonsBody(showNames:boolean)}
    ...
    <button class="flex items-center hover:text-primary transition-colors" onclick={async () => {
        ...
        changeChatTo(0)
        saveNewChatToServer(DBState.db.characters[selIdState.selId].chaId, 0, newChat.id, newChat as any).catch(() => {})
    }}>
        <SplitIcon size={20}/>
        {#if showNames}
