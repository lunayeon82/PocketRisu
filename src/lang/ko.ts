...
export const languageKorean = {
    ...
    "errors": {
        ...
        "networkFetchWeb": "이것은 CORS 오류일 수 있습니다. 이는 브라우저의 제한으로 인해 웹 버전을 사용할 때만 발생합니다. 데스크톱 로컬 버전이나 다른 버전의 PocketRisu를 사용해보세요.",
    ...
    "setup": {
        ...
        "welcome": "PocketRisu에 오신 것을 환영해요! 저는 PocketRisu 셋업을 도와줄 아이리스라고 해요. 먼저 닉네임을 입력해 주세요!",
        "welcome2": "{username}님 안녕하세요! PocketRisu를 시작하려면 몇 가지 설정만 하시면 되요. Ai 제공자를 선택해 주세요!",
    ...
    "playground": {
        ...
        "inlayImageGalleryEmptyDesc": "채팅에서 첨부 또는 생성된 이미지가 여기에 나타납니다",
    ...
    "noSwipesRerollConfirm": "현재는 추가 재생성된 메세지가 없습니다. 재생성하여 신규 메세지를 목록에 추가 할까요?",
    "chatList": "채팅 리스트",
    ...
    "characterPackageEmptyWarning": "캐릭터 데이터가 비어 있습니다. 그래도 임포트할까요?",
    "characterPackageNameMismatch": "캐릭터 이름이 다릅니다. 그래도 임포트할까요?",
    "characterPackageImportToChar": "캐릭터에 패키지 불러오기",
    "characterPackageLicenseWarning": "라이선스 정책으로 캐릭터를 내보낼 수 없습니다",
    ...
    "updatePopupDesc": "새 버전 <b>v{{latest}}</b>를 사용할 수 있습니다. (현재: v{{current}})",
    ...
    "chatWidthWide": "넓게",
    "chatWidthFull": "제한 없음",
    "customCSSWarning": "PocketRisu는 원본 RisuAI와 UI 구성(HTML/CSS)이 다르기 때문에, 기존에 쓰던 커스텀 CSS가 레이아웃을 깨거나 의도와 다르게 동작할 수 있습니다. 어떤 부분을 덮어쓰는지 정확히 아는 경우에만 사용하세요.",
    "customHTMLWarning": "PocketRisu는 원본 RisuAI와 UI 구성(HTML/CSS)이 다르기 때문에, 기존에 쓰던 커스텀 Chat HTML이 정상적으로 렌더되지 않을 수 있습니다. 어떤 부분을 덮어쓰는지 정확히 아는 경우에만 사용하세요.",
    ...
    "inlayCompressConfirm": "모든 인레이 이미지를 WebP로 압축합니다. 되돌릴 수 없습니다. 계속하시겠습니까?",
    ...
    "saveBackupForUpstreamConfirm": "이 백업은 원본 RisuAI(웹·Tauri)로 데이터를 옮기기 위한 용도입니다. 인레이 이미지는 entry 이름 형식이 원본 RisuAI 임포트와 호환되지 않아 제외됩니다. 그 외 데이터(데이터베이스, 캐릭터 에셋, 콜드 스토리지)는 포함됩니다.\n\n포켓리스 사이의 데이터 이동에는 일반 로컬 백업을 사용하세요.\n\n계속하시겠습니까?",
    ...
    "shareExport": "공유/엑스포트",
    "risupresetDesc": "Risupreset 포맷은 포맷을 공유하기 위한 PocketRisu 전용 포맷입니다.",
    ...
    "importTranslationCacheFailed": "{0}개 항목을 가져왔지만 {1}개 항목 저장에 실패했습니다. 저장 공간이 가득 찼을 수 있습니다.",
    ...
    "pluginVersionWarn": "이 플러그인은 {{plugin_version}} 버전입니다. 이 버전의 PocketRisu와 호환되지 않습니다. 플러그인을 {{required_version}} 버전으로 업데이트하세요.",
    ...
    "pluginRiskDetectedAlert": "PocketRisu가 API 키나 개인 데이터를 훔칠 수도 있는 위험한 코드를 감지했습니다. 이 플러그인을 설치하지 않는 것을 권장합니다.",
    "doNotInstall": "설치하지 않음",
    "continueAnyway": "무시하고 계속",
    "pluginNoRiskButAlert": "PocketRisu가 위험한 코드를 감지하지 못했지만, 신뢰할 수 없는 출처의 플러그인을 설치하면 보안 위험이 발생할 수 있습니다.",
    ...
    "bookmark": "북마크",
    "branch": "분기점",
    "fetchLogConsent": "플러그인 {} 이(가) 로그를 가져오려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?",
    "getFullDatabaseConsent": "플러그인 {} 이(가) 전체 데이터베이스에 접근하려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?",
    "mainDomAccessConsent": "플러그인 {} 이(가) 메인 Document에 접근하려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?",
    "replacerPermissionConsent": "플러그인 {} 이(가) 채팅 내용을 교체할 권한을 요청합니다. 대화 조작에 사용될 수 있습니다. 허용하시겠습니까?",
    "providerPermissionConsent": "플러그인 {} 이(가) Provider 접근 권한을 요청합니다. 무단 API 호출이 발생할 수 있습니다. 허용하시겠습니까?",
    "sendChatConsent": "플러그인 {} 이(가) 사용자 대신 채팅 메시지를 전송할 권한을 요청합니다. AI 응답이 트리거됩니다. 허용하시겠습니까?",
    "resetPluginPermission": "권한 응답 초기화",
    "resetPluginPermissionConfirm": "플러그인 \"{}\"의 저장된 권한 응답(허용/거부 이력)을 모두 삭제하시겠습니까?\n\n다음에 플러그인이 권한을 요청할 때 다시 묻게 됩니다.",
    "resetPluginPermissionDone": "\"{}\"의 권한 응답을 초기화했습니다",
    "pluginV2Warning": "플러그인 V2 및 V2.1은 안전하지 않다고 간주되며, 향후 버전에서는 동작이 중단됩니다. **이 버전의 플러그인을 사용하지 마세요.** 이 플러그인의 개발자라면 가능한 한 빨리 V3로 업데이트해주세요.",
    ...
    "serverBackupDesc": "다운로드 없이 포켓리스가 설치된 기기(서버)에 직접 백업 파일을 저장하고 복원합니다.",
    "serverBackupSave": "서버에 백업 저장",
    "serverBackupManage": "서버 백업 관리",
    "serverBackupSaving": "서버에 백업 저장 중...",
    "serverBackupSaveSuccess": (filename: string, size: string) => `백업 저장 완료: ${filename} (${size})`,
    ...
    "serverBackupDeleteConfirm": (filename: string) => `"${filename}" 백업을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`,
    ...
    "togglePresetDeleteConfirm": (name: string) => `"${name}" 토글 프리셋을 삭제할까요?`,
    ...
    "togglePresetEmptyFiltered": "현재 프롬프트 프리셋에 해당하는 토글 프리셋이 없습니다.",
    "togglePresetApplyConfirm": "현재 토글 설정이 덮어씌워집니다. 적용하시겠습니까?",
    "togglePresetMismatchConfirm": "현재 프리셋과 다릅니다. 토글이 호환되지 않을 수 있습니다. 적용하시겠습니까?",
    ...
    "togglePresetRenamed": (oldName: string, newName: string) => `"${oldName}" → "${newName}"으로 변경되었습니다.`,
    ...
    "importSaveZipDesc": "사용하던 RisuAI 프로젝트의 save 폴더를 zip으로 압축한 뒤 여기에 업로드하면 데이터를 불러올 수 있습니다. 압축 파일 용량이 큰 경우 업로드에 실패할 수 있으며, 이 경우엔 서버의 save 폴더에 직접 복사한 후 재시작하면 자동으로 마이그레이션됩니다.",
    "importSaveFolderScanning": "Save 폴더 스캔 중...",
    "importSaveFolderImporting": "Save 폴더 불러오는 중...",
    "importSaveFolderConfirmZip": (name: string, size: string) => `"${name}" (${size})을 업로드하고 불러올까요? 현재 데이터를 대체합니다.`,
    "importSaveFolderNoDatabase": "Save 폴더에 데이터베이스 파일(database/database.bin)이 없습니다. 불러오기를 진행할 수 없습니다.",
    "importSaveFolderSuccess": "불러오기 완료.",
    "cleanupMigratedFiles": "마이그레이션된 Save 파일 정리",
    "cleanupMigratedDesc": "save 폴더를 직접 덮어씌워 마이그레이션한 경우, 이미 데이터베이스에 저장된 원본 파일을 삭제하여 디스크 공간을 확보합니다.",
    "cleanupMigratedConfirm": (count: number, size: string) => `마이그레이션 완료된 파일 ${count}개 (${size})를 삭제할까요? 이미 데이터베이스에 저장되어 있습니다.`,
    "cleanupMigratedNoFiles": "정리할 파일이 없습니다.",
    "cleanupMigratedNotReady": "마이그레이션이 아직 완료되지 않았습니다.",
    "cleanupMigratedSuccess": (count: number, size: string) => `${count}개 파일 삭제, ${size} 확보.`,
    ...
    remoteAccess: "원격 접속",
    remoteAccessDesc: "원격 접속 링크를 사용하여 다른 기기에서 이 포켓리스 서버에 접속할 수 있습니다.",
    ...
    storageDiskRisuTotal: (size: number) => `PocketRisu 합계: ${(size / 1024 / 1024).toFixed(1)} MB`,

    // 항목별 라벨 (분포 리스트의 ⓘ 설명)
    storageRowDbFile: "risuai.db",
    storageRowDbFileDesc: "PocketRisu의 메인 SQLite 데이터베이스 파일. 모든 채팅·캐릭터·에셋·설정이 여기 들어갑니다.",
    ...
    storageInternalOnlyHint: "끄면 PocketRisu 내부 항목만, 켜면 기타 시스템·여유 공간까지 디스크 전체 기준으로 표시합니다.",
    ...
    // 데이터 이전 (구 "계정 & 파일")
    migration: "데이터 이전",
    migrationDesc: "원본 RisuAI(또는 호환 시스템)에서 데이터를 가져오거나 PocketRisu 데이터를 원본 호환 형식으로 내보냅니다.",
    ...
    migrationSaveFolderDesc: "이전에 사용하던 노드리스/RisuAI 인스턴스의 save 폴더를 PocketRisu로 가져올 때 사용합니다.",
